import {
  ...
  isoToday, formatDE, exportAll, importAll
} from "./db.js";
...
function svgChartTime(labels, seriesA, seriesB, { height = 300, type = "line", yLabel = "Score" } = {}) {
  const w = 900;
  const h = height;

  // bottom padding groß wegen gedrehter Datumslabels
  const padL = 62;
  const padR = 18;
  const padT = 16;
  const padB = 92;
  ...
  // X Labels: alle Tage, -45°
  let xLabels = "";
  const xBaseY = h - padB + 18;
  for (let i = 0; i < labels.length; i++) {
    const xx = x(i);
    const lbl = formatDE(labels[i]);
    xLabels += `
  ...
  // bar: values as bars, MA as dashed line
  // line: both as lines (MA dashed)
...
function svgCategoryTotalsBar(names, totals, { height = 320, yLabel = "Score (Summe)" } = {}) {
  const w = 900;
  const h = height;

  // bottom padding groß wegen -45° Kategorienlabels
  const padL = 72;
  const padR = 18;
  const padT = 16;
  const padB = 140;
  ...
  const bw = Math.max(3, Math.min(26, step * 0.7));
  ...
  for (let i = 0; i < totals.length; i++) {
    ...
    xLabels += `
      <text x="${xc}" y="${xLabelY}" font-size="10" fill="#475569"
        text-anchor="end"
        transform="rotate(-45 ${xc} ${xLabelY})">${escapeXml(lbl)}</text>
    `;
  }

  return `
    <div style="overflow:auto;">
      <svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" role="img" aria-label="Kategorie-Summen">
        ${grid}
        ${axes}
        ${yAxisLabel}
        ${yTicks}
...
async function renderHeatmap(fromIso, toIso) {
  ...
  for (const d of days) {
    ...
    if (e) {
      ...
      cell.innerHTML = `<div class="d">${formatDE(d)}</div><div class="s">${total}</div>`;
    } else {
      cell.innerHTML = `<div class="d">${formatDE(d)}</div><div class="s">—</div>`;
    }
...
async function renderTimeChart() {
  ...
  chartTitle.textContent =
    `${name} · ${formatDE(fromIso)} – ${formatDE(toIso)} · ${modeSel.value === "daily" ? "täglich" : "kumuliert"}`;
...
async function renderCategoryTotalsChart() {
  ...
  // sync mode: copy from chart 1 range
  ...
  // Summe pro Kategorie (nur aktive Kategorien anzeigen)
  ...
  catChartTitle.textContent = `Summe pro Kategorie · ${formatDE(fromIso)} – ${formatDE(toIso)}`;
...
async function renderAll() {
  ...
  // Heatmap folgt dem Zeitraum des ersten Charts
  const fromIso = fromInp.value;
  const toIso = toInp.value;
...
async function init(skipEnsure = false) {
  ...
  const onChangeCat = async () => {
    if (catPresetSel.value === "custom") {
      // keep user-defined dates
    }
    await renderCategoryTotalsChart();
  };
