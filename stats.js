...
/* -------------------- Chart 1: Axis + Plot -------------------- */
function chartTimeAxisSvg({ height, yLabel, yMin, yMax, ticksCount, y }) {
  const axisW = 76;
  const h = height;
  const padT = 10;
  const padB = 76;
  const padR = 8; // little inner padding

  let gridTicks = "";
  for (let i = 0; i <= ticksCount; i++) {
    const val = yMin + (i * (yMax - yMin) / ticksCount);
    const yy = y(val);
    gridTicks += `<text x="${axisW - padR}" y="${yy + 4}" font-size="11" fill="#475569" text-anchor="end">${Number.isInteger(val) ? val : val.toFixed(1)}</text>`;
  }

  const axisLine = `<line x1="${axisW-1}" y1="${padT}" x2="${axisW-1}" y2="${h-padB}" stroke="#cbd5e1" stroke-width="1.2" />`;

  const label = `
    <text x="16" y="${(padT + (h - padB)) / 2}" font-size="12" fill="#475569"
      transform="rotate(-90 16 ${(padT + (h - padB)) / 2})">${yLabel}</text>
  `;

  return `
    <svg viewBox="0 0 ${axisW} ${h}" width="${axisW}" height="${h}" aria-hidden="true">
      ${axisLine}
      ${label}
      ${gridTicks}
    </svg>
...
function chartTimePlotSvg(labels, seriesA, seriesB, { height = 280, type = "line" } = {}) {
  const w = 900;
  const h = height;

  // Plot-only padding (Y-axis is separate)
  const padL = 12;
  const padR = 16;
  const padT = 10;
  const padB = 76;
  ...
  const xAxis = `<line x1="${padL}" y1="${h - padB}" x2="${w - padR}" y2="${h - padB}" stroke="#cbd5e1" stroke-width="1.2" />`;
  ...
  `, yMin, yMax, y, ticksCount, height: h };
}

function svgChartTimeSticky(labels, seriesA, seriesB, { height = 280, type = "line", yLabel = "Score" } = {}) {
  const plot = chartTimePlotSvg(labels, seriesA, seriesB, { height, type });
  const axisSvg = chartTimeAxisSvg({
    height: plot.height,
    yLabel,
    yMin: plot.yMin,
    yMax: plot.yMax,
    ticksCount: plot.ticksCount,
    y: plot.y
  });
  ...
  return `
    <div class="chartFlex">
      <div class="chartAxis">${axisSvg}</div>
      <div class="chartScroll">${plot.svg}</div>
    </div>
    ${legend}
  `;
}
...
/* -------------------- Chart 2: Axis + Plot -------------------- */
function chartCatAxisSvg({ height, yLabel, yMin, yMax, ticksCount, y }) {
  // identisch wie Chart 1 axis
  return chartTimeAxisSvg({ height, yLabel, yMin, yMax, ticksCount, y });
}

function chartCatPlotSvg(names, totals, { height = 290 } = {}) {
  const w = 900;
  const h = height;
...
function svgCategoryTotalsSticky(names, totals, { height = 290, yLabel = "Score (Summe)" } = {}) {
  const plot = chartCatPlotSvg(names, totals, { height });
  const axisSvg = chartCatAxisSvg({
    height: plot.height,
    yLabel,
    yMin: plot.yMin,
    yMax: plot.yMax,
...
async function renderTimeChart() {
  ...
  chartEl.innerHTML = svgChartTimeSticky(days, series, ma, {
...
async function renderCategoryTotalsChart() {
  ...
  catChartEl.innerHTML = svgCategoryTotalsSticky(names, totals, { yLabel: "Score (Summe)" });
}
