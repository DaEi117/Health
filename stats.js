...
let categories = [];       // includes archived (for mapping ids)
...
async function renderCategoryHeatmap(fromIso, toIso) {
  ...
  // WICHTIG: Diese Breiten müssen zu .catHeatCell (26px) passen
  const DATE_W = 78;
  const CELL_W = 26;
  const FLAG_W = 26;
  ...
  grid.style.gridTemplateColumns = `${DATE_W}px repeat(${cats.length}, ${CELL_W}px) ${FLAG_W}px ${FLAG_W}px`;
  ...
  for (const c of cats) {
    ...
    // rotLbl wird via CSS absolut in die Spaltenmitte gesetzt -> perfekt aligned
